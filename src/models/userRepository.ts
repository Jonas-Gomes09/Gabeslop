import {Usuario} from "../entities/comentario"
...
export class userRepository {
    ...
    private async loadUsers(): Promise<Usuario[]> {
        try {
            const content = await readFile(this.usersFile, "utf-8");
            const parsedContent = JSON.parse(content)
            return parsedContent.map((d: any) => {Usuario.fromJSON(d)})
        } catch {
    ...
    private async saveUsers(users: Usuario[]): Promise<void> {
        try {
    ...
    async addUser(titulo: string, usuario: string, nota: number, dataCriacao: string, totalCompras: number, foto: string | null = null): Promise<Usuario> {
        const erros = Usuario.validar({ titulo, usuario, nota})
        ...
        const newUser = new Usuario(nextID, titulo, usuario, nota, dataCriacao, totalCompras = 0, foto)
        users.push(newUser)
    ...
    async listAll(searchTerm?: string): Promise<Usuario[]> {
        let users = await this.loadUsers()
        if (searchTerm && searchTerm.trim()) {
            const lowercase = searchTerm.toLowerCase()
            users = users.filter(u => u.titulo.toLowerCase().includes(lowercase))
        }
        return users
    }

    // Procurar por ID (PAINEL DE MODERADOR / INFORMAÇÕES DA CONTA DO CLIENTE)
    async userInfo(id: number): Promise<Usuario | undefined> {
        const users = await this.loadUsers()
